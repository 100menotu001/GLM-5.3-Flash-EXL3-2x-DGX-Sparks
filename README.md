...
| `GLM53_MIXED_PREFILL_CHUNK` | `skip` (`start.sh` / `start-tp4.sh`); `0` on `start-tp3.sh` | Mixed-prefill policy while a peer is decoding (issue #6). **`skip` starves every waiting/running prefill until that decode ends** (no age limit). `N>0` caps mixed chunks to N tokens and feeds N into hybrid Mamba alignment so N < block_size still progresses. `0`/`off` disables isolation. **`fair` is opt-in** time-share mixing (decode-only stretches, then at most `GLM53_FAIR_PREFILL_MAX_CHUNKS` chunks of `GLM53_FAIR_PREFILL_CHUNK` tokens, gated by GPU-time share and `GLM53_FAIR_PREFILL_MAX_INTERVAL_MS`). Do not make `fair` the default until `tests/test_mixed_prefill_decode.py` overlap gates pass on this kit. Solo prefill stays MNBT (7168). Patch installer is versioned (`# [glm53-decode-floor:v2]`) so an already patched image migrates. |
| `GLM53_FAIR_PREFILL_CHUNK` | `256` | Fair mixed-turn token cap. Keep well below MNBT so decode rows still fit. Forwarded on every rank even when CHUNK is not `fair`. |
| `GLM53_FAIR_PREFILL_SHARE` | `0.20` | Target fraction of recent GPU time for prefill while mixed. `0..1`. |
| `GLM53_FAIR_PREFILL_MAX_INTERVAL_MS` | `2000` | Maximum wait between prefill opportunities while a decode peer exists. |
| `GLM53_FAIR_PREFILL_MAX_CHUNKS` | `1` | Prefills admitted on one mixed turn (round-robin / aging). |
...
| `overlay/patch_scheduler_decode_floor.py` | skip / cap / off / opt-in `fair` mixed-prefill; versioned v2 installer + alignment-aware cap |
| `tests/test_scheduler_decode_floor.py` | v2 patch + v1 migration, skip/cap/off/fair policy, alignment zeros vs policy cap |
