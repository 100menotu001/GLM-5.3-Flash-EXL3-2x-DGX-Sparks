...
| `GLM53_HOST_MEM_HYGIENE` | `1` | drop clean page cache on both nodes right before `docker run`, then wait for CUDA free memory to clear `GPU_MEM_UTIL` × total (`sudo -n`; warns without it). On GB10 UMA the page cache is "used" CUDA memory: a full cache after the 164 GiB rsync made InstantTensor abort (`buffer_size … exceeds device memory budget`) or crawl at a double-digit `io_depth`. Pairs with `overlay/patch_cold_load_uma.py` (in-container budget + 64 KiB mmap staging; kill switch `GLM53_COLD_LOAD_UMA=0`; no-op on 4 KiB kernels / discrete GPUs). See [docs/cold-load-uma.md](docs/cold-load-uma.md) |
