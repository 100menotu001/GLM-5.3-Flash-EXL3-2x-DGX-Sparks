...
| `GLM53_MIXED_PREFILL_CHUNK` | `fair` on `start.sh` (since 2026-09-15); `skip` on `start-tp4.sh`; `0` on `start-tp3.sh` | Mixed-prefill policy while a peer decodes. **`skip` starves prefills until decode ends** (the reported multi-minute newcomer freeze). `N>0` caps mixed chunks with hybrid alignment support; `0`/`off` disables isolation (admits newcomers in ~1 s but collapses the incumbent 10–36×). `fair` v5 allocates decodes first, charges only prefill that contends with a decoder, fits a fixed-plus-per-token step cost, runs the largest chunk that fits `GLM53_FAIR_PREFILL_MAX_STEP_MS`, and gives a newcomer one prompt probe. Measured on this kit (reporter recipe, thinking essay at ~24 tok/s): 2k newcomer first token 9 s, 30k newcomer 146 s while the essay still streams, incumbent keeps ~80–87% of its in-run rate with p95 gap ~0.2 s and max ~1–1.5 s. Migration supports v1..v4. See [receipts](docs/diditfix.md) and [design](docs/astra-fix.md). TP=4 is untested with `fair`. |
