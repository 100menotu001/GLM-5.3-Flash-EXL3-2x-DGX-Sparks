...
- Scoped auto/lazy safetensors mmap staging (`GLM53_LOAD_CLONE=1`) and optional
  bounded local-shard read-ahead (`GLM53_LOAD_PREFETCH=0`, decimal `0..16`) on
  TP2/TP3/TP4. Controls reject malformed or empty values before restart and
  reach every rank; loader defaults, quantization and InstantTensor are unchanged.
  TP3's read-only vendored loader is generated by the same overlay, with an early
  stale-artifact check. CPU tests cover exact bytes, lifecycle/errors and PR #230
  composition in both orders; no GPU speed, KV-capacity or quality result is claimed.
