...
CompletionRequest separately defaults max_tokens to 16, so the completion
caller uses Pydantic field-set metadata to distinguish omission from an
explicit 16. With DEFAULT_MAX_NEW_TOKENS empty/unset both paths stay stock.
...
All target transformations are validated before writing either file.
"""
...
def main(argv: list[str]) -> int:
    ...
    if raw and not (raw.isascii() and raw.isdecimal() and 1 <= int(raw) <= 1000000):
        ...
        return 2
    changes = []
    for relative, transform in ((LIMITS_PATH, apply_text), (COMPLETION_PATH, apply_completion_text)):
