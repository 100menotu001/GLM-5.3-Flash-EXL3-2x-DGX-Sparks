...
**Current alignment correction (CPU-qualified, not GPU-measured).** The final
splitter also bounds alignment capacity by the actual `num_new_tokens` grant,
not just configured, threshold and fair-policy limits. The earlier distinction
between intentional caps and temporary residual budget is superseded: a small
residual can recur indefinitely, so rounding each positive grant to zero can
starve prefill. CPU execution of the patched scheduler now makes positive
sub-block progress under those grants without enlarging them or changing
fairness/admission policy. It still stops at shared-LCM checkpoints, the last
reusable checkpoint and the final hash-grain prompt tail. Unequal Mamba layouts
also stop at the next necessary smaller private-page boundary so unfinished
recurrent state is not crossed or published prematurely.

This liveness correction may create more small mixed steps rather than waiting
for full-page capacity; its GPU throughput and decode-latency effects are
unmeasured. The CPU proof covers actual splitter transitions and cache-manager
allocation/prefix metadata, not numerical GPU state, copy-on-write or logit
parity. Live measurements below remain historical, not measurements of this
correction; final reviewer approval is not asserted. See the
[current APC description and CPU recipe](../README.md#fine-grained-hybrid-apc-alignment-and-replay).
...
It did not know the overlay’s smaller mixed cap. At a block boundary, if the prompt needed more chunks and `mixed_cap < block_size <= max_prefill_tokens`, the capped chunk rounded down to zero. A waiting request then hit the alignment branch’s `break`; a running prefill took the zero-token `continue`. Repeating this decision could reproduce starvation even with a positive mixed cap. This was an additional hazard when choosing a fix, not the explanation for the reporter’s `CHUNK=0` runs. The current grant-aware correction is described above.

Historical CPU execution of the extracted, uncorrected alignment method used request/configuration stubs. With MNBT 7168 and an illustrative 3584-token block:
...
4. **Make the small-chunk policy compatible with hybrid alignment.** Pass the effective policy chunk limit into the alignment decision, or otherwise explicitly allow the base implementation’s private sub-block state progression under that limit. Preserve mandatory cache boundaries, partial-tail handling, and resumed-request replay. Do not remove alignment or publish an incomplete recurrent state as a reusable prefix entry. The original requirement distinguished an intentional policy cap from temporary residual capacity after other requests consumed a step budget; the final correction supersedes that distinction by clamping to the actual grant as well, with smaller private-Mamba-page protection. Otherwise a persistently small residual could repeat zero progress indefinitely.
